...
export JAVA_HOME=`/usr/libexec/java_home -v 17` # for specified java version
...
alias dockerffmpeg="docker run --rm -v $(pwd):$(pwd) -w $(pwd) jrottenberg/ffmpeg"
...
if [[ `ls | grep pyproject` =~ pyproject.toml ]]; then
    #echo -e "You are in poetry directory"
    poetry shell
fi

alias testchrome="~/Downloads/chrome-mac-arm64/Google\ Chrome\ for\ Testing.app/Contents/MacOS/Google\ Chrome\ for\ Testing --ignore-certificate-errors"
